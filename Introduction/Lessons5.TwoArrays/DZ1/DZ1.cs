       ...
       if (ValidatePosition(numbers, x, y))
        {
            ...
            Console.WriteLine($"Значение элемента по позиции ({x}, {y}): {result}");
        }
        else
        {
...
class Answer
{
    ...
    {   
        ...
        if (args.Length >= 3)
        {
            // Предполагается, что строки разделены запятой и пробелом, а элементы 
            //внутри строк разделены пробелом
