...
# максимальное количество ягод, которое может собрать собирающий модуль, находясь перед некоторым кустом грядки.
...
max_berry= 0
for i in range(n):
    max_berry = max( max_berry, arr[i-1] + arr[i] + arr[i-2] )
print(max_berry)